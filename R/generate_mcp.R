...
#' @param include_api_tools Logical. Include API server start/stop tools.
#'   Default: FALSE.
...
#'
#' @export
generate_mcp_tools <- function(
  pkg_path,
  output_path,
  pkg_name = NULL,
  include_api_tools = FALSE,
  verbose = TRUE
) {
  ...
  if (verbose) {
    ui_info(glue::glue(
      "\U0001F916 Generating MCP tool definitions for '{pkg_name}'"
    ))
  }
  ...
  if (length(exports) == 0) {
    if (verbose) {
      ui_warn("No exported functions found -- MCP schema will be minimal")
    }
  }
  ...
  schema <- list(
    ...
    resources = build_mcp_resources(
      pkg_name,
      pkg_path,
      include_api_tools = include_api_tools
    ),
  ...
  if (verbose) {
    ...
    for (tool in tools) {
      ui_info(glue::glue(
        "  \U0001F527 {tool$name}: {substr(tool$description, 1, 60)}..."
      ))
    }
  }
...
build_mcp_tool <- function(fn_name, fn_meta, pkg_name) {
  ...
  list(
    ...
    outputSchema = build_mcp_output_schema(fn_meta),
    annotations = build_mcp_annotations(
      read_only = FALSE,
      destructive = FALSE,
      idempotent = FALSE
    ),
    dataimago = build_dataimago_tool_metadata(fn_meta),
    implementation = list(
      type = "nextjs_api",
...
#' Build conservative MCP output schema metadata
#' @noRd
build_mcp_output_schema <- function(fn_meta) {
  list(
    type = "object",
    description = if (
      !is.null(fn_meta$return_doc) && nzchar(fn_meta$return_doc)
    ) {
      fn_meta$return_doc
    } else {
      "JSON-serializable response from the R package function."
    },
...
#' Build MCP protocol annotations
#' @noRd
build_mcp_annotations <- function(read_only, destructive = FALSE, idempotent) {
  list(
...
#' Build dataimago-specific tool safety metadata
#' @noRd
build_dataimago_tool_metadata <- function(
  fn_meta,
  prototype = NULL,
  uses_private_data = FALSE,
  requires_approval = TRUE,
  safe_for_static_export = FALSE
) {
  if (is.null(prototype)) {
    text <- paste(fn_meta$title, fn_meta$description, fn_meta$return_doc)
    prototype <- grepl("\\bprototype\\b", text, ignore.case = TRUE)
  }

  list(
    prototype = isTRUE(prototype),
    usesPrivateData = isTRUE(uses_private_data),
    requiresApproval = isTRUE(requires_approval),
    safeForStaticExport = isTRUE(safe_for_static_export)
  )
}
...
build_api_start_tool <- function(pkg_name) {
  list(
    ...
    outputSchema = list(
      type = "object",
      description = "Status information for starting the local live-producer server.",
      additionalProperties = TRUE
    ),
    annotations = build_mcp_annotations(
      read_only = FALSE,
...
#' @noRd
build_mcp_resources <- function(pkg_name, pkg_path, include_api_tools = FALSE) {
  resources <- list(
    list(
    ...
    list(
      ...
      description = "OpenAPI 3.0 specification served by the canonical NextJS API",
      uri = "/api/openapi.json"
    )
  )

  if (isTRUE(include_api_tools)) {
    resources <- c(
      resources,
      list(list(
        name = "live_api_openapi_spec",
        description = "OpenAPI 3.0 specification served by a local live R API server",
...
#' @noRd
build_mcp_capabilities <- function(exports, include_api_tools = FALSE) {
  list(
    data_access = list(
      read = TRUE,
      query = TRUE,
    ...
    api_services = list(
      nextjs_api = TRUE,
      live_rest_api = isTRUE(include_api_tools),
      openapi_spec = TRUE,
      cors_enabled = isTRUE(include_api_tools)
    )
  )
}
