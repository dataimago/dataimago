...
// src/brand/Logo.tsx
import { svgs } from "@dataimago/brand";
...
function BrandAnimation({
  ...
  const entry = animationManifest.find((a) => a.id === name);
  let markup = animations[name];
  if (!entry || !markup) {
...
function NavbarLogo({ name, width, style, ...rest }) {
  ...
  const entry = navbarManifest.find((n) => n.id === name);
  let markup = navbarLogos[name];
  if (!entry || !markup) {
